...
def getMealMenu(today="",period=""):
    ...
    return {"mealMenu":tmp,"image_url":image_url} 


def school_schedule():
    
    ret ="다음은 제주과학고등학교 2023학년도  학교 행사 또는 학사 일정이다. 학사 일정은  '행사날자: 행사명'로 구성되어 있다."
    ret +="학교행사 또는 학사일정 등에 질문울 받으면 다음 데이터를 참고하여 대답한다. \n학사 일정 시작"
    section=[1,2] 
    for hakgi in section:
        ...
        for b in bap:
            ...
            if  ':' in b.text:
                ...
                if t[0] != t[1] : #시작 일자와 끝 일자가 다른 경우
                    tt=t[0] + "~" + t[1] + ":" + t[2] 
                else: #시작 일자와 끝 일자가 같은 경우는 시작 일자와 행사명 출력
                    tt=t[0] + ":" + t[2]     
                ret += tt + ", "   
    return ret.replace("&","-")  + "학사일정 종료."

if __name__ == "__main__": #main이라는 것은 이 모듈을 직접 불렀을 때
    ret=school_schedule()
    print(ret)
    print("="*100)
    ret=getMealMenu("","")
